...
# Valset address
valsetAddr="$contractAddr"

# Current distribution config
currentDistribution=$(tgrade query wasm contract-state smart "$valsetAddr" "{ \"configuration\": {} }" -o json --node="$nodeUrl" | jq '.data.distribution_contracts')

# Replace engagement by tc payments in distribution
newDistribution="${currentDistribution//$engagementAddr/$tcPaymentsAddr}"

migrateMsg="{
  \"distribution_contracts\": $newDistribution
}"
echo "Migrate msg:"
echo "$migrateMsg" | jq '.'
