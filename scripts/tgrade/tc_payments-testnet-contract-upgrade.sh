...
originalCodeId=26 # From ./list_contracts.sh
contract="tgrade_tc_payments"

# Get address (Warning: migrating only last contract instance)
contractAddr=$(tgrade q wasm list-contract-by-code "$originalCodeId" -o json --node="$nodeUrl" | jq -r '.contracts[-1]')
...
title="Migrate $contract contract to code id $newCodeId"
description="Migrate '$contract' contract with address '$contractAddr' to code id '$newCodeId'"
...
echo "$contract upgrade proposal: $upgrade_proposal"
...
# Dump contract configuration
echo "$contract configuration:"
tgrade query wasm contract-state smart "$contractAddr" "{ \"configuration\": {} }" -o json --node="$nodeUrl" | jq '.'
