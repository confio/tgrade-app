...
export default function ProposalWhitelistPair({
  ...
  useEffect(() => {
    (async function getPairs() {
      ...
      const tokensPerPairsWithDso = tokensPerPairs.filter(
        (pair) => pair.tokenA.dsoAddress === ocAddress || pair.tokenB.dsoAddress === ocAddress,
      );

      setTokensPerPairs(tokensPerPairsWithDso);
    })();
  }, [client, config.factoryAddress, ocAddress, getPairToken, signingClient]);
  ...
  async function submitCreateProposal() {
    ...
    if (!ocAddress || !signingClient || !client || !address) return;
    setSubmitting(true);
